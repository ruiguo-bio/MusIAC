...
def cal_key(piano_roll, key_names,end_ratio=1):
...
def merge_tension(metric, beat_indices, down_beat_indices,window_size=-1):
    ...
    if window_size == -1:
        ...
        # else:
        #     if (len(metric) - down_beat_indices[-1]) > 0:
        #         new_metric.append(np.mean(metric[down_beat_indices[i+1]:],axis=0))
...
def cal_tension(piano_roll,beat_time,beat_indices,down_beat_time,down_beat_indices, window_size=-1, key_name='',
                sixteenth_time=None,pm=None):
    try:
        ...
        if len(down_beat_time) > 9999999:
            ...
            key_change_bar = detect_key_change(key_diff,diameters)
            if key_change_bar != -1:
            ...
            key_name,key_pos, note_shift = cal_key(piano_roll,key_name)
            changed_note_shift = -1
            changed_key_name = ''
            key_change_beat = -1
            change_time = -1
...
if __name__== "__main__":
    ...
    for file_name in all_names:
        ...
        # logger.info(f'working on {file_name}')
        result = extract_notes(pm,args.track_num)
